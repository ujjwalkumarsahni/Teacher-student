...
export const getStudentResults = async (req, res) => {
  try {
    ...
    const results = attempts.map((attempt) => {
      ...
      const status = attempt.score >= totalMarks * 0.33 ? "PASS" : "FAIL";
...
export const getStudentExamResult = async (req, res) => {
  try {
    ...
    const wrongAnswers = detailedAnswers.filter(
      (a) => !a.isCorrect && a.selectedOption !== undefined,
    ).length;
  ...
  try {
    ...
    const attempt = await ExamAttempt.findById(attemptId)
      ...
      .populate({
        ...
        model: "Question",
        select:
          "questionText options marks negativeMarks correctAnswer difficultyLevel topic",
      })
    ...
    const allQuestions = await Question.find({ exam: attempt.exam._id }).select(
      "questionText options marks negativeMarks correctAnswer difficultyLevel topic",
    );
    ...
    const detailedAnswers = allQuestions.map((question) => {
      ...
      const isCorrect =
        answered && answered.selectedOption === question.correctAnswer;
    ...
    const statistics = {
      ...
      incorrect: detailedAnswers.filter((a) => a.isAttempted && !a.isCorrect)
        .length,
